...
Matrix_T mw_matrix_add(const Matrix_T *first_mat, const Matrix_T *second_mat)
{
    ...
    {
        mw_temp_matrix_object.matrix[i] = first_mat->matrix[i] + second_mat->matrix[i];
    }
...
VectorT mw_matrix_mult(const Matrix_T *mat_inp, const VectorT *vec_inp)
{
    ...
    mw_temp_vector_object.vector[0] =
        (mat_inp->matrix[0] * vec_inp->vector[0]) + (mat_inp->matrix[2] * vec_inp->vector[1]);

    mw_temp_vector_object.vector[1] =
        (mat_inp->matrix[1] * vec_inp->vector[0]) + (mat_inp->matrix[3] * vec_inp->vector[1]);
...
VectorT mw_matrix_mult(const Matrix_T *mat_inp, const VectorT *vec_inp)
{
    ...
    {
        mw_temp_vector_object.vector[i] = (mat_inp->matrix[i] * vec_inp->vector[0]) +
...
VectorT mw_matrix_mult(const Matrix_T *mat_inp, const VectorT *vec_inp)
{
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        mw_temp_vector_object.vector[i] =
            (mat_inp->matrix[i] * vec_inp->vector[0]) + (mat_inp->matrix[i + 4] * vec_inp->vector[1]) +
            (mat_inp->matrix[i + 8] * vec_inp->vector[2]) + (mat_inp->matrix[i + 12] * vec_inp->vector[3]);
    }
...
VectorT mw_matrix_mult(const Matrix_T *mat_inp, const VectorT *vec_inp)
{
    ...
    {
        mw_temp_vector_object.vector[i] =
            (mat_inp->matrix[i] * vec_inp->vector[0]) + (mat_inp->matrix[i + 5] * vec_inp->vector[1]) +
            (mat_inp->matrix[i + 10] * vec_inp->vector[2]) + (mat_inp->matrix[i + 15] * vec_inp->vector[3]) +
...
MatrixT mw_matrix_mult_full_classic(const MatrixT *mat_inp1, const MatrixT *mat_inp2)
{
    ...
    for (i = 0; i < rows_used; ++i)
    {
        for (k = 0; k < inner_dim; ++k)
        {
            ...
            {
                mw_temp_matrix_object.matrix[i][j] += mat_inp1->matrix[i][k] * mat_inp2->matrix[k][j];
            }
        }
    }
