...
static Ret_T
kf_init_vect_2_vect_copy(VectorT *kf_vect_result, const float32_t kf_vector_inp[], const uint32_t kf_len_of_vec);
...
VectorT kalman_filter_computation(const Kalman_Filter_T *kf_vectors_inp, VectorT *kf_vector_x_k_1_inp)
{
    ...
    for (size_t i = (kf_vectors_inp->control_signal_inp.rows);
         i < (kf_vectors_inp->control_signal_inp.rows + kf_vectors_inp->y_meas_inp.rows);
         ++i)
    {
...
static VectorT kf_first_line_of_state_space(const VectorT *kf_state_vec_inp, const VectorT *kf_u_signal_inp)
{
    ...
    kf_matrix_mult_temp1.vector[0] = (d_matrix_A_DU.matrix[0] * kf_state_vec_inp->vector[0]) +
                                     (d_matrix_A_DU.matrix[2] * kf_state_vec_inp->vector[1]);

    kf_matrix_mult_temp1.vector[1] = (d_matrix_A_DU.matrix[1] * kf_state_vec_inp->vector[0]) +
                                     (d_matrix_A_DU.matrix[3] * kf_state_vec_inp->vector[1]);

    kf_matrix_mult_temp2.vector[0] = (d_matrix_B_plus_K_DU.matrix[0] * kf_u_signal_inp->vector[0]) +
                                     (d_matrix_B_plus_K_DU.matrix[2] * kf_u_signal_inp->vector[1]);

    kf_matrix_mult_temp2.vector[1] = (d_matrix_B_plus_K_DU.matrix[1] * kf_u_signal_inp->vector[0]) +
...
static VectorT kf_second_line_of_state_space(const VectorT *kf_state_vec_inp, const VectorT *kf_u_signal_inp)
{
    ...
    /* Manually unrolling the for-loop */
    kf_matrix_mult_temp1.vector[0] = (d_matrix_C_DU.matrix[0] * kf_state_vec_inp->vector[0]) +
                                     (d_matrix_C_DU.matrix[2] * kf_state_vec_inp->vector[1]);

    kf_matrix_mult_temp1.vector[1] = (d_matrix_C_DU.matrix[1] * kf_state_vec_inp->vector[0]) +
                                     (d_matrix_C_DU.matrix[3] * kf_state_vec_inp->vector[1]);

    kf_matrix_mult_temp2.vector[0] =
        (d_matrix_D_DU.matrix[0] * kf_u_signal_inp->vector[0]) + (d_matrix_D_DU.matrix[2] * kf_u_signal_inp->vector[1]);

    kf_matrix_mult_temp2.vector[1] =
        (d_matrix_D_DU.matrix[1] * kf_u_signal_inp->vector[0]) + (d_matrix_D_DU.matrix[3] * kf_u_signal_inp->vector[1]);
...
static VectorT kf_first_line_of_state_space(const VectorT *kf_state_vec_inp, const VectorT *kf_u_signal_inp)
{
    ...
    {
        kf_matrix_mult_temp1.vector[i] = (d_matrix_A_DU.matrix[i] * kf_state_vec_inp->vector[0]) +
                                         (d_matrix_A_DU.matrix[i + 3] * kf_state_vec_inp->vector[1]) +
                                         (d_matrix_A_DU.matrix[i + 6] * kf_state_vec_inp->vector[2]);
        kf_matrix_mult_temp2.vector[i] = (d_matrix_B_plus_K_DU.matrix[i] * kf_u_signal_inp->vector[0]) +
...
static VectorT kf_second_line_of_state_space(const VectorT *kf_state_vec_inp, const VectorT *kf_u_signal_inp)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        kf_matrix_mult_temp1.vector[i] = (d_matrix_C_DU.matrix[i] * kf_state_vec_inp->vector[0]) +
                                         (d_matrix_C_DU.matrix[i + 3] * kf_state_vec_inp->vector[1]) +
                                         (d_matrix_C_DU.matrix[i + 6] * kf_state_vec_inp->vector[2]);
        kf_matrix_mult_temp2.vector[i] = (d_matrix_D_DU.matrix[i] * kf_u_signal_inp->vector[0]) +
